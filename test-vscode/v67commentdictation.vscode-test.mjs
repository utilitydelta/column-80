// session-v67: dictating INTO a comment in a real extension host. The microphone AND the
// recogniser are fixtures (helpers/fake-mic.cjs, helpers/fake-recogniser.cjs); the editor, the
// gesture, the comment insert and the hand-off to Tighten Doc Comment are real. The tighten's
// own outcome is recorded, not asserted: this tier has no instruct model, so its gate refuses.
//
// Run, one label at a time, with nothing else on the box (a parallel run turned C# red in v66):
//   npm run build
//   DISPLAY=:1 npx vscode-test --config test-vscode/v67commentdictation.vscode-test.mjs --label ts
// On the editor installed on this box:
//   C80_VSCODE=/usr/share/code/code DISPLAY=:1 npx vscode-test --config test-vscode/v67commentdictation.vscode-test.mjs --label ts
...
const scratch = process.env.C80_SCRATCH ?? path.join(os.tmpdir(), 'c80-v67');
...
const base = {
  ...
  files: 'v67-comment-dictation.test.js',
