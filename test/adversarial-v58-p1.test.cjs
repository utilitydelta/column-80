...
test("CLEAN [src/vscode]: every channel tag a comment names is a tag some non-comment line in src emits", () => {
  ...
  // the bound 20 became 21; session-v65 added `[dictate]`, so 21 became 22: the
  // scan keeps the same one slot of slack it was written with, and a loosened
  // regex still trips it.
  assert.ok(
    emitted.size < 22,
