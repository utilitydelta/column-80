...
| `column80.injectedSurface` | `auto` | How much type detail goes into the function-generation prompt. `auto` is the measured default, tuned for a small local model. `generous` raises it for a frontier model or a large local one; `minimal` lowers it. Prompt size only, never the number of language-server lookups, so turning it up does not slow the editor. |
