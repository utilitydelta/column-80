...
`cargo check --all-targets --keep-going --message-format=json`, run against disk, parsed directly. **`--all-targets` since session-v69 (supersession S33).** It was plain `cargo check`, whose named trade was that `#[cfg(test)]` bodies sat outside the oracle's sight. That trade was correct while the oracle only ever checked generated FUNCTION BODIES, and became wrong the moment the product started WRITING `#[cfg(test)] mod tests`: it validated its own test code with a command blind to it, reported clean, and repair honestly did nothing. `--all-targets` rather than the narrower `--tests` because `--tests` skips the plain lib build, so a `#[cfg(not(test))]` error the old command reported would VANISH - a new way to report clean, which is the defect being closed. Go's command moved the same way and for the same reason: `go test -c -o os.devNull ./...`, which compiles the test binary and never runs it. `--keep-going` arrived WITH the widening and not before it: more targets means cargo can stop starting units once one has failed, and one unrelated broken example was measured swallowing the generated test's own error entirely. **Toolchain floors: cargo 1.74 for `--keep-going`, Go 1.21 for multi-package `go test -c`.** An older toolchain refuses the command in plain text, which parses to zero diagnostics and routes to `describeCheckFailure` - an honest crash, never a quiet green. Go's refusal is `go: `-prefixed and so needed `GO_USAGE_REFUSAL`, an allow-list that keeps real `go: ` VERDICTS (`updates to go.mod needed`, `inconsistent vendoring`) as diagnostics while dropping cmd/go refusing its own command line. The VS Code diagnostics API is never read: flycheck runs against unsaved buffers and its flattening drops the byte spans and `suggested_replacement` data repair needs. Feeding it back in would launder stale data into model calls. Display flows one way, core to screen: an annotation at the edit site, and the output channel. This extension publishes no diagnostics of its own, because owning one means owning its lifecycle and the language server already does that for the same errors.
