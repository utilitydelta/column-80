...
Consequence: one ~200-line client instead of three, and the seam stays a plain function. The trade is that native-only features (Anthropic prompt caching, provider tool-calling fidelity, reasoning-model knobs) are unreachable, and reasoning-model quirks like `max_completion_tokens` vs `max_tokens` are the user's problem to match via model choice. Fn-gen is prompt-to-text with no tools, so none of that is load-bearing today.
