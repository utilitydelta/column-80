...
| `FIM_NUM_CTX` | 8192 | `src/core/budgetProfile.ts` | ollama's context window on EVERY FIM request (session-v65) | DERIVED: the 3000/1000 windows are about 1,300 tokens, the largest dictation surface stop measured is 1,200, a rambling take is a few hundred, `maxTokens` is 256; unpinned, ollama's default took 3,369 tokens whole and cut 5,000 to 2,050 on this box, from the HEAD of the prefix. Pinned on every request, not only the dictated ones, because the server reloads the model when `num_ctx` changes between requests: the host tier measured that reload at about 2s while another client of the same model kept sending the default. KV cache at 8192 on the 1.5b is about 235MB (28 layers x 2 KV heads x 128 x K,V x f16), inside the carve |
| `INTENT_INJECTION_DEADLINE_MS` | 400 | `src/core/completionService.ts` | the injection race for a DICTATED request | JUDGMENT CALL against the bar: a dictated request has a second from mic close to ghost, and 400 leaves decode (about 250ms) plus FIM p50 (184ms) inside it on the reference box. The keystroke race stays 50 |
| `DICTATION_SURFACE_TOK` | 300 | `src/core/budgetProfile.ts` | the render budget for the surfaces a dictated request injects | PROVEN-ARM in the negative: 300/600/1200 on 360 sites gave 145/143/142 first lines against 157 with no surface at all (noise floor 5), flat to downward. The number stays because the leg is ruled on and switchable (`column80.dictation.surfaces`); no budget rescues it |
| `PARTIAL_WINDOW_MS` / `PARTIAL_INTERVAL_MS` | 6000 / 300 | `src/vscode/dictation.ts` | the trailing window a streaming partial decodes, and how often | MEASURED-NARROW: the recogniser spike held a 300ms loop on a six second window at mean 232ms, worst 260ms, on the reference box; a longer window costs decode time per tick and was not tried |
| `BEAM_SIZE` | 5 | `src/core/recogniser.ts` | whisper.cpp beam search on every decode | PROVEN on one fixture: greedy heard "THREKT LEVEL", beam 5 heard "threat level", at the same 250ms; quick-speech's daily setting |
| `VAD_MIN_SILENCE_MS` | 500 | `src/core/recogniser.ts` | Silero VAD's split threshold | INHERITED from quick-speech's daily use; VAD itself is PROVEN on silence (3s of zeros decoded to "You" without it, to nothing with it) |
| `ACCEPT_GRACE_MS` | 300 | `src/vscode/dictation.ts` | how long a cursor move off the dictated line is held after an edit on it | DERIVED from the editor's ordering: the accept's cursor move arrives before the accept command, measured in the host tier |
| `START_TIMEOUT_MS` / `SETTLE_MS` | 20000 / 200 | `src/core/recogniser.ts` | how long the resident recogniser may take to answer, and the settle after an answer that tells a bind failure from a stranger on the port | JUDGMENT CALL; the server comes up in about 80ms on this box, a bind failure exits within a few ms |
